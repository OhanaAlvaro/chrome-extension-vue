  ...
  <div style="min-width: 300px;">
    ...
    >
      ...
      <span v-else-if="data.shield == `done`" style="color: #00b359">
        ...
        <span v-if="scenesCountByCategory.reduce((x, a) => x + a.selected, 0) == 0">
          This movie does not contain any unwanted scenes.
        </span>
        <span v-else>
          We will skip {{ scenesCountByCategory.reduce((x, a) => x + a.selected, 0) }}
          scenes to filter out all the content you have marked as unwanted.
        </span>
      </span>
      <span v-else-if="data.shield == `unknown`" style="color: orange">
        ...
        We might not be able to skip all the content you have marked as unwanted.
      </span>
      <span v-else style="color: red">
        <v-icon small color="red" class="mb-1">mdi-shield-alert</v-icon>
        <b>Unsafe movie!</b> <br />
        <!--Users reported there are scenes not yet filtered, so we can't skip all your unwanted
        content.-->
        <!--We won't be able to protect you.-->
        This movie includes content you have marked as unwanted, but we don't have the filters to
        filter it out, yet. Please consider
        <a href="https://www.patreon.com/ohanamovies">donating</a> to encourage the creation of
        filters or <a @click="showSidebar(true)">becoming an editor yourself!</a>.
      </span>
    </div>
    ...
    <div>
      ...
      <!-- TODO: uncomment this when ready<v-row>
        <v-col class="py-0">
          <v-checkbox
            class="my-0"
            dense
