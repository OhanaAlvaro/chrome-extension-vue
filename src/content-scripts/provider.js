...
var provider = {
	...
	getLinks: function() {
		...
		if (host.includes('netflix')) {
			...
			if (false && window.location.pathname.includes('/busqueda/')) {
				// it places icon on the wrong place: position relative kind of fixes it, but we would have to change mroe things...
				links = document.querySelectorAll('[href*="/ficha/"]')
			} else {
				links = document.querySelectorAll('.enlaceFicha')
			}
		}
		return links
	},
